...
@Service
public class OptionMentorService {
    ...
    public OptionResponseDTO createOption(OptionCreateDTO createDTO) {
        ...
        // Verify if the question belongs to the current mentor
        if (!question.getMentor().getId().equals(currentUser.getId())) {
            throw new SecurityException("You can only add options to your own questions");
        }
    ...
    public OptionResponseDTO getOption(String optionId) {
        ...
        // Verify if the option belongs to the current mentor's question
        if (!option.getQuestion().getMentor().getId().equals(currentUser.getId())) {
            throw new SecurityException("You can only view your own options");
        }
    ...
    public List<OptionResponseDTO> getOptionsByQuestion(String questionId) {
        ...
        if (!question.getMentor().getId().equals(currentUser.getId())) {
            throw new SecurityException("You can only view options from your own questions");
        }
    ...
    public OptionResponseDTO updateOption(String optionId, OptionCreateDTO updateDTO) {
        ...
        // Verify ownership
        if (!option.getQuestion().getMentor().getId().equals(currentUser.getId())) {
            throw new SecurityException("You can only update your own options");
        }
    ...
    public void deleteOption(String optionId) {
        ...
        // Verify ownership
        if (!option.getQuestion().getMentor().getId().equals(currentUser.getId())) {
            throw new SecurityException("You can only delete your own options");
        }
